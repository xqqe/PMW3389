...
int readAxis(int thisAxis) {
  ...
  // map the reading from the input range to the output range:
  reading = map(reading, 0, upperLimit, 0, range);
...
void UpdatePointer(void){
  if(initComplete==9){
    ...
    ///////////
    new_xydat[0] = readAxis(convTwosComp(xydat[0]));
    new_xydat[1] = readAxis(convTwosComp(xydat[1]));
    ///////////
...
void loop() {
  ...
  if(movementflag){
    ...
    /////////// mouse command based on readAxis, from joystick mouse example
    Mouse.move(new_xydat[0],new_xydat[1]);

    Serial.print("n_x = ");
    Serial.print( new_xydat[0] );
    Serial.print(" | ");
    Serial.print("n_y = ");
    Serial.println( new_xydat[1] );
    ///////////
